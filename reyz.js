    ...
    client.on('message', async (msg) => {
     if(msg.body.startsWith('!google ')) {
        ...
        try{
            ...
            const quer = msg.body.slice(6)
            const r = await yts(quer)
            //if ( err ) throw err
            const vi = r.videos[0]
            const link = vi.url
            //console.log(link)
            const title = vi.title
            const desk = vi.description
            const thum = vi.thumbnail
            const durasi = vi.timestamp
            if (durasi > '6:00') {console.log('duration is so long')}
            else {
                ryz.youtube(link).then(result => {
                    const url = result.mp3
...
client.on('message', async (msg) => {
    const chat = await msg.getChat()
    const chatid = chat.id._serialized
    if(msg.body === '!tebakgambar') {
        if(grupchat.includes(chatid)) {
            try {
                msg.reply('ada sesi yang belum di selesai kan !')
            }catch (err) {
                console.log(err)
            }
        }
            ...
            const jawab = result[0].jawaban
            const jjson = {
                gcid:chatid,
                ans:jawab
            }
            grupchat.push(chatid)
            jawaby.push(jjson)
            console.log(grupchat)
            try {
                const start = async function(a, b) {
                    const media = await MessageMedia.fromUrl(image)
                    await chat.sendMessage(media, {caption:'hayo jawab, reply pesan ini ya jika dah tau jawaban nya. Waktu kamu 60 detik'})
                    setTimeout( () => {
                        if(grupchat.includes(chatid)) {msg.reply(`waktu habis *${jawab}*`)}
                        grupchat.splice(grupchat, 1)
                        jawaby.splice(jjson, 1)
                    }, 30000)
                    setTimeout( () => {
                        if(grupchat.includes(chatid)) {msg.reply(`10 detik lagi`)}
                        else{clearTimeout(start)}
                    }, 20000)
                    setTimeout( () => {
                        if(grupchat.includes(chatid)) {msg.reply(`20 detik lagi`)}
                        else{clearTimeout(start)}
                    }, 10000)
                    setTimeout( () => {
                        if(grupchat.includes(chatid)) {
                            msg.reply(`30 detik lagi`)
                            }
                        else{clearTimeout(start)}
                    }, 1000)
                }
    ...
    }else if(msg.body === '!nyerah') {
        console.log('1')
        if(grupchat.includes(chatid)) {
            for(let jawb of jawaby) {
                console.log('1')
                const answ = jawb.ans
                if(jawb.gcid === chatid) {
                    const jjson = {
                    ...
                    jawaby.splice(jjson, 1)
                    try {
                        msg.reply(`telah menyerah *${answ}*, ketik !tebakgambar jika ingin main lagi`)
                    }catch (err) {
                        console.log(err)
                    }
                }
    ...
    else if(grupchat.includes(chatid)) {
        for(let jawb of jawaby) {
            if(jawb.gcid === chatid) {
                const answ = jawb.ans
                const answer = answ.split('Jawaban ')[1]
                console.log(answer)
                if (msg.body.includes(answer)) {
                    const jjson = {
                        gcid:chatid,
                        ans:answ
                    }
